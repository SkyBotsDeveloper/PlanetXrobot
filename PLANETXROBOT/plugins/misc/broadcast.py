...
from PLANETXROBOT.misc import SUDOERS, mongodb
...
from PLANETXROBOT.utils.decorators.language import language
from PLANETXROBOT.utils.formatters import alpha_to_int
...
async def scan_broadcast_chats(client, message):
    ...
    status = await message.reply_text("Syncing broadcast targets from Planetx DB...")
    ...
    lines = [
        ...
        "This command syncs broadcast targets directly from the Planetx Mongo database.",
        "",
        f"Mongo collections scanned: {mongo_sync['collections']}",
        f"Mongo documents scanned: {mongo_sync['scanned_docs']}",
        f"Chat/channel IDs found in Planetx DB: {mongo_sync['found_chats']}",
        f"Private user IDs found in Planetx DB: {mongo_sync['found_users']}",
...
async def braodcast_message(client, message, _):
    ...
    if "-assistant" in message.text:
        ...
        text = _["broad_6"]
        from PLANETXROBOT.core.userbot import assistants
