...
from PLANETXROBOT.mongo.welcomedb import is_on, set_state, bump, cool, auto_on

BG_PATH = "PLANETXROBOT/assets/planetx/welcome.png"
FALLBACK_PIC = "PLANETXROBOT/assets/upic.png"
FONT_PATH = "PLANETXROBOT/assets/planetx/Arimo.ttf"
...
async def welcome(client, update: ChatMemberUpdated):
    ...
    try:
        ...
        for f in (avatar, img):
            if f and os.path.exists(f) and "PLANETXROBOT/assets" not in f:
                try:
                    os.remove(f)
                except:
                    pass