...
import config
from PLANETXROBOT import LOGGER, app, userbot
from PLANETXROBOT.core.call import JARVIS
from PLANETXROBOT.misc import sudo
from PLANETXROBOT.plugins import ALL_MODULES
from PLANETXROBOT.utils.database import get_banned_users, get_gbanned
from PLANETXROBOT.utils.lyrics import close_lyrics_http_client
...
BOT_COMMANDS = [
    ...
    BotCommand("save", "Save a group note"),
    BotCommand("lock", "Lock a content type"),
    BotCommand("setflood", "Configure antiflood"),
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("PLANETXROBOT.plugins" + all_module)

    LOGGER("PLANETXROBOT.plugins").info("Modules loaded.")
    LOGGER("PLANETXROBOT").info(
    ...
    await idle()
    try:
        weather_plugin = importlib.import_module("PLANETXROBOT.plugins.Kishu.weather")
        await weather_plugin.close_weather_http_client()
    except Exception:
        pass
    try:
        song_plugin = importlib.import_module("PLANETXROBOT.plugins.tools.song")
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("PLANETXROBOT").info("Stopping music bot...")
